...
int main(void){
    InitWindow(500,500,"fsdfc");
    SetTargetFPS(60);
    int N = QQTY/2;
    FluidCube * cube = FluidCubeCreate(QQTY,1,1,1.0/60.0);
    FluidCubeAddDensity(cube,1,1,0,255.0);
    while(!WindowShouldClose()){
        BeginDrawing();
        ClearBackground(RAYWHITE);
        for(int y=0;y<QQTY;y++){
            for(int x=0;x<QQTY;x++){
                Color c = (Color) {255,0,0,cube->density[IX(x,y,0)]};
                DrawRectangle(x*10,y*10,9,9,c);
             }
          }
        //FluidCubeStep(cube);
        EndDrawing();
