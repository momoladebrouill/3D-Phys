...
let f _ y y' args = 
  ...
  Graph.mapi (fun i p -> 
    if trd p.pos < 0.1  then 
      begin 
        y.(i) <- (fst p.pos, snd p.pos, 0.1);
        y'.(i) <- (fst p.vit, snd p.vit, -. trd p.vit *. c0)
      end;
    (somme_forces (bilan_des_forces p i (volume args.l) points args) p ) /$ p.mass  ) points
